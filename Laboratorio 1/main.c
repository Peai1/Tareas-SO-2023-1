...
/*
Función que busca "palabra_buscar" en la matriz que contiene la sopa de letras, esto se realiza recorriendo la matriz y cuando se encuentre
con la primera letra de la palabra a buscar, obtiene las siguientes letras de la palabra candidata a ser la que se desea buscar, siempre y cuando
tenga la misma longitud de la palabra a buscar. Luego de obtener la palabra candidata se compara con la palabra a buscar y sigue asi hasta que
la encuentra en la sopa de letras
Parametros:
        ...
        for (int j = 0; j < columnas; j++){         
            if ( matriz[i][j] == palabra_buscar[0] ){
                if ( j + longitud <= columnas) { // Para asegurar que no nos salgamos de la fila
                    ...
                    palabra_comparar[longitud] = '\0'; // Se agrega el carácter nulo al finalS
...
int main(){
    ...
    char sopa[200][200];
    char path[1024];    // Para borrar .txt
    ...
    if ((carpeta = opendir("Archivos")) == NULL)
        ...
        while ((leido = readdir(carpeta)) != NULL) {
            ...
            if (strcmp(leido->d_name, ".") != 0 && strcmp(leido->d_name, "..") != 0){
                ...
                if (obj == NULL){
                    // Carpeta no ha sido creada todavia
